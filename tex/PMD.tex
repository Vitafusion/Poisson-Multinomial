...
\begin{equation*}
    \sqrt{b} \left(\overline{Y}_{\xvec}-p(\xvec) \right)\xrightarrow{d} \textrm{N}(0,\sigma^2).
\end{equation*}
...
\begin{align*}
    & \sum_{\xvec \in \chi}\EE |\overline{Y}_{\xvec}-p(\xvec)| = c \sum_{r=1}^{h} \sqrt{p(\xvec)(1-p(\xvec))}  = c h \sum_{\xvec\in\chi}\frac{\sqrt{p(\xvec)(1-p(\xvec))}}{h} \\
