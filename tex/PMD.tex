...
Suppose a random variable $\Xvec =  (X_1, \dots, X_{m})' \sim \PMD(\Pmat)$, where $\Pmat = (p_{ij}), i=1,\dots,n,j=1,\dots,m$. Let $X_m = n-\sum_{j=1}^{m-1}X_j$ and the cf between $\Xvec$ and $\Xvec^{\star} = (X_1, \dots, X_{m-1})'$ will be equivalent. The cf of $\Xvec^{\star}$ is 
