...
\usepackage{latexsym,epsfig,graphicx,epstopdf,amsmath,amssymb,amscd,multirow,chicago,psfrag,paralist,dsfont,url,apacite}
...
\begin{document}
	...
	%%%%%%%%%%%%TITLE%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title{\texttt{PMD}: An R package for Computing the Probability Mass Function for the Poisson-Multinomial Distribution}
	%\iffalse
\author{Zhengzhi Lin\quad Virginia Tech \\ Yueyao Wang \quad Virginia Tech \\
	Yili Hong\quad Virginia Tech}
	%\fi
...
The Poisson Multinomial Distribution (PMD) is defined as sum of different independent multinomial distributions. It has applications in game theory \cite{Cheng2017PlayingAG}, digital imaging \cite{akter2019double},machine learning \cite{kamath2014learning} .  One of the popular areas that involved PMD is voting theory. A very simple example is, suppose we have $n$ independent voters to vote for a president from $m$ candidates, each voter has its own chance to vote on a certain candidate. It is not our concern how voters vote, we care only the voting result, that is how many votes each candidate gets. An $(n,m)$ PMD is a perfect model for this example. In machine learning, especially in classification context, when we classify $n$ samples one by one into $m$ categories, then the total number of samples assigned to each category follows a PMD. When $(n,m)$ is small it is possible to calculate the distribution function by using enumeration method. However when $n,m$ get large, we need better method, or algorithm to calculate it. Some former studies have uncover PMD's structure and properties, \cite{diakonikolas2016fourier} shows the Fourier transformation of PMD is sparse and provide a theorem that there exist algorithms to calculate PMD's density. \cite{Daskalakis2015OnTS} also prove us PMD is $\epsilon$-cover and Central Limit Theory is valid for PMD. Other papers such as \cite{akter2019double} shows us some interesting application of PMD and its sparsity property. Due to the huge practical value of PMD, computing its pmf is of great importance. However, there is no available algorithm for computing pmf for PMD. In this case, we are motivated to develop a thorough method to calculate its pmf. We design three methods to calculate it,
\begin{enumerate}[Method 1]
    \item MD-DFT(base on multi-dimensional discrete Fourier transform(MD-DFT)) method: we use Multidimensional Fast Discrete Fourier Transformation algorithm to calculate PMD's exact probability mass function.
    \item Simulation based method: we use multinomial distributions to simulate the process of voting and use the frequency as our result to approximate true result.
    \item Normal approximation based method: when $n$ is large, we apply CLT and use a approximated normal distribution as our result.
\end{enumerate}
...
Note that when the SPM is identical across all rows, that is, $I_{i}$, $i = 1, \dots n$ are identically distributed, the distribution of $X$ can be simplified as multinomial distribution. Hence, the PMD is a generalization of the multinomial distribution. When $m=1$, the PMD is reduced to the Poisson binomial distribution as in \cite{hong2013computing}.

In related literature, \cite{hong2013computing} consider the exact and approximate methods for computing the pmf of the Poisson binomial distribution. \cite{zhang2018generalized} introduce the general Poisson binomial distribution and develop an algorithm to compute its distribution functions.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The MD-DFT Method Multidimensional Fourier Transform}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\\
To get all $p(x_1,\dots,x_{m-1})$ with respect to given $n$, $m-1$ and $P_{n \times (m-1)}$, we apply the Fast Fourier Transformation algorithm(FFT) from GSL Scientific Library to make calculation efficient. This FFT algorithm is C language based, we implemented it and wrote a new MD-DFT algorithm. Our MD-DFT algorithm can calculate all values of distribution function as long as we input our $P_{n\times (m-1)}$ matrix, and it can be called from R.
...
\hline
\end{tabular}
    \caption{Accuracy of MD-DFT method}
    \label{tab:my_label}
\end{table}
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.8]{(n,m)pairs.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.9]{normal accuracy.png}
...
\KwResult{Write here the result }
 ...
 \begin{itemize}
     \item compute the frequency of every result that stored in R as our density point value.\\
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.8]{simu accuracy.png}
...
In this section, we show computation efficiency of simulation method and MD-DFT for calculating all density points as $(n,m)$ gets large. We generate random matrices with respect to given (n,m), record the calculating time of each method. As for normal approach, it is a asymptotic method, we only need to calculate the asymptotic normal distribution.
\begin{table}[h!]
    \centering
\begin{tabular}{|c|c|c|}
...
\hline
\end{tabular}
    \caption{Accuracy of simulation method}
    \label{tab:my_label}
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\section{Examples of Using PMD in Heart Disease Data}\label{sec:model.est.inf}	
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
